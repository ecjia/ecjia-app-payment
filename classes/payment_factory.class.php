...
class payment_factory  
{
	...
	public function set_adapter($adapter_name, $adapter_config = array()) 
	{
		if (!is_string($adapter_name)) 
			...
			$payment_plugins = ecjia_config::instance()->get_addon_config('payment_plugins', true, true);
		    if (isset($payment_plugins[$code])) {
		        RC_Plugin::load_files($payment_plugins[$code]);
		    }
			$this->adapter_instance =  RC_Hook::apply_filters('payment_factory_adapter_instance', null, $adapter_config); 
	...
	public function __call($method_name, $method_args) 
	{
		...
		elseif (
			...
			&& ($this->adapter_instance instanceof payment_abstract)
