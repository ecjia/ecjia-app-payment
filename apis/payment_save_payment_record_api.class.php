...
class payment_save_payment_record_api extends Component_Event_Api {
	...
	public function call(&$options) {	
		if (!array_get($options, 'order_sn') || !array_get($options, 'pay_code') || !array_get($options, 'pay_name') || !array_get($options, 'order_amount')) {
			return new ecjia_error('invalid_parameter', RC_Lang::get('payment::payment.invalid_parameter'));
		}
		
		/* 插入支付流水记录 */
		$db = RC_DB::table('payment_record');
		$payment_record = $db->where('order_sn', $options['order_sn'])->first();
		$payment_data = array(
		...
		if (empty($payment_record)) 
		{
            ...
            $payment_record['pay_code'] != $options['pay_code'] && 
		    $options['order_amount'] != $payment_record['total_fee']) {
